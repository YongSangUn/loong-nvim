...
return {
  ...
  -- custom end --
  --- On first initialization, you must wait for mason to install powershell-editor-services before the LSP can start correctly
  cmd = function(dispatchers)
    ...
    local command_fmt = "& '%s/PowerShellEditorServices/Start-EditorServices.ps1' "
      .. "-BundledModulesPath '%s' "
      .. "-LogPath '%s/powershell_es.log' "
      .. "-SessionDetailsPath '%s/powershell_es.session.json' "
      .. "-FeatureFlags @() "
      .. "-AdditionalModules @() "
      .. "-HostName nvim -HostProfileId 0 -HostVersion 1.0.0 -Stdio -LogLevel Normal"
